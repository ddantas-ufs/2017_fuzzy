...
void metodo(FILE *entrada,int nImg,int type,int tam, int bh, float ts, float x){
  int i;
  char diretorio[200];
  ...
  for(i = 0; i<nImg;i++){
    ...
    //float *element = gaussianFunction2D2( tam, tam, ( (tam) / 2.0) );
    //float *element = gaussianFunction2D2( tam, tam, ( (tam) / 4.0) );   
...
printf(" ***Tamanho %d, Jan: %dx%d, Val(x,y): %dx%d, TS: %f ***\n", atoi(argv[3]), atoi(argv[5]), atoi(argv[6]), atoi(argv[7]), atoi(argv[8]), atof(argv[9]) );
  ...
  metodo(entrada, n, ident, atoi(argv[3]), atoi(argv[4]), atof(argv[5]), atof(argv[10]));
